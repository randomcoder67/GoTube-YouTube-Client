...
func StartPlayback(title string, channel string, startTime string, startNum string, folderName string, quality string, geometryArg string) {
	mpvCommandArgs := []string{"--title=" + title + " - " + channel, "--start=" + startTime, "--playlist-start=" + startNum, "--script=" + youtube.HOME_DIR + "/.local/bin/gotube.lua", "--resume-playback=no", "--geometry=" + geometryArg, "--script-opts=gotube-folderName=" + folderName + ",gotube-quality=" + quality, "--playlist=" + folderName + "/playlist.m3u"}
